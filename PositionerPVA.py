...
import sys
import os
# sys.path.insert(0,"/APSshare/anaconda3/x86_64/lib") #adds parent directory to path so importing from other directories work. 
import configparser
import time
import copy
import h5py
...
class PositionerServer(PvaServer):
	def __init__(self, config_file="PositionerPVA.cfg"):
		...
		savepath = "./output.h5"
		self.output_file_path = savepath
	...
	def _update_path(self, data):
		name = self.output_file_path.split("/")[-1]
		if not data["value"].endswith("/"):
			data["value"] +="/"
		self.output_file_path = data["value"]+name
		
	def _update_name(self, data):
		path = "/".join(self.output_file_path.split("/")[:-1])
		if not path.endswith("/"):
			path +="/"
		self.output_file_path = path+data["value"]
